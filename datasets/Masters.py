...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Masters dataset.
...
class MastersDataset(PointCloudDataset):
    """Class to handle Masters dataset."""

    def __init__(self, config, set='train', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'Masters')
        ...
        self.cloud_names = ['train', 'validate']
        self.all_splits = [0, 1]
        ...
        elif self.set in ['validate']:
            self.epoch_n = config.validation_size * config.batch_num
        else:
            raise ValueError('Unknown set for Masters data: ', self.set)
        ...
        if use_potentials:
            ...
            # self.epoch_inds = torch.from_numpy(np.zeros((2, 300), dtype=np.int64))
...
class MastersSampler(Sampler):
    """Sampler for Masters"""

    def __init__(self, dataset: MastersDataset):
...
class MastersCustomBatch:
    """Custom batch definition with memory pinning for Masters"""
...
def MastersCollate(batch_data):
    return MastersCustomBatch(batch_data)
